...
def download_vids(video_list_df, output_dir,trim = False):
    ...
    for i, row in video_list_df.iterrows():
        try:
            ...
            print("Downloaded ... Task:{} Id:{}".format(row['task'], row['vid_id']))
        except subprocess.CalledProcessError:
            print("Error ... Task:{} Id:{}".format(row['task'], row['vid_id']))
        ...
        if trim:
            ...
            subprocess.call(['ffmpeg', '-ss', str(row['start_seconds']), '-i', original_vid_path, '-t', str(duration), '-c:a', 'ac3', '-c:v', 'libx264', '--', vid_path])
            os.remove(original_vid_path)
            print("Trimed ... Task:{} Id:{}".format(row['task'], row['vid_id']))
...
if __name__ == '__main__':
    ...
    errors = download_vids(video_list_df, args.output_dir,args.trim)
