...
if [ "$flg_s" = "TRUE" ]; then
  if [ "$flg_c" = "TRUE" ]; then
    for task in ${tasks[@]}
      do
        python main.py ${args[0]} ${args[1]} $task --lap ${args[2]} --split $var_s --cuda $var_c
      done
  else
    for task in ${tasks[@]}
      do
        python main.py ${args[0]} ${args[1]} $task --lap ${args[2]} --split $var_s
      done
  fi
elif [ "$flg_c" = "TRUE" ]; then
  for task in ${tasks[@]}
    do
      python main.py ${args[0]} ${args[1]} $task --lap ${args[2]} --cuda $var_c
    done
else
  for task in ${tasks[@]}
    do
      python main.py ${args[0]} ${args[1]} $task --lap ${args[2]}
    done
fi