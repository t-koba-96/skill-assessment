...
def save_checkpoint(state, file_path, epoch, score, is_best):
    ...
    if is_best:
        ...
        best_name = os.path.join(file_path, "best_score(epoch_{:04d}).ckpt".format(epoch))
        shutil.copyfile(savefile, best_name)
        print(('Saving checkpoint file ... best_score(epoch_{:04d}).ckpt'.format(epoch)))
